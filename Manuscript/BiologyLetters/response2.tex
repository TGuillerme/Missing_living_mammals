...
Dear Surayya Johar,\\
\bigskip
%We are very happy on the manuscript acceptance decision. % NC: Not usual to write this, of course we are!
...
% NC: Bit weird to have referee two and then 1, I've moved it back so it's in the right order! always easier for an editor to read like that than to have to go back and forth.
...
% NC: Fix these. And check the supp material.
...
lines @@@.
% NC: I've edited this slightly. Note that the issue is that even though we might find loads of data for a living mammal, if it is all soft tissue it is USELESS! It cannot overlap with fossil taxa. In an ideal world we would be just looking for hard characters, but we've done this now so instead we will have to mention this in the discussion somewhere

We also added a comment in the discussion to the effect that having data for a living mammal doesn't necessarily mean it can overlap with a fossil - it could all be soft tissue characters. However, in general our datasets appear to contain at least one hard character for each living species so it shouldn't influence our results. But yes it is an important consideration for future work and we thank the reviewer for pointing this out (we misunderstood this point in the first round of revisions). % NC: We did misunderstand this first go round. This is why I don't like you to rush. You need to learn how to very carefully read a reviewers letter and understand the criticisms. Remember one paragraph of comments might have several things to fix.

% NC: Need to add a line or two for this in the discussion. Or maybe rephrase the above and we can just stick with the clarifications I've added to the methods and intro.

% NC: I've changed a few bits in the text too
...
% NC: I've editted this a bit to get more closely at what the reviewer wants (it thinks OUR analysis might be biased, and is correct).
% Make sure you also write a brief explanation here as well as editting the text. Perhaps add a line like:
...
%NC: Also need to add a line sayong we've now mentioned this in the discussion and add the bit here.
