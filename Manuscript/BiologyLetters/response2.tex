...
% NC: I've rewritten this to make it clearer what you have done and to stop it reading like "yeah we know there is a bias but we are too lazy to fix it". Now it says "yeah we know there is a bias but we are too lazy to fix it, but we also think it isn't a big bias and would add more bias anyway". A much better justification.
We acknowledge this comment (and Referee 3 comment 2) and have now made more explicit reference to this bias. 
However, because we did not collect these matrices, we are unable to properly quantify the bias introduced by excluding them.
To test this we would need to collect these data and then rerun the analysis.
This would really interesting but would also take a substantial amount of time to collect the data, which is not available at present. 
Additionally we do not believe this bias will be as large as the reviewer fears.
Many recent matrices reuse characters from older matrices, thus it is likely most of the data from major previous studies are present in our analyses.
We also share the concerns of Peter Wagner that using ``old'' matrices may introduce a different kind of bias into phylogenies, as definitions of homology and characters have changed over time, making the kind of matrix we are unlikely to find with our search protocol harder to incorporate into Total Evidence analyses.
We therefore modified the paragraph discussing this bias as follows:
