...
Dear Surayya Johar,\\
\bigskip
We are very grateful to the four referees for their helpful and constructive comments. %, which we believe have helped us to significantly improve our paper. % NC: A bit too much for what are minor revisions :)
...
% NC: It's not just the first 10 results per search term is it? I thought you went through the first 10 pages of results? Clarify this.
...
Following these two observations, we think that including the 20 most recent results % NC: Is that what you did?
...
% NC: You replace something *with* something else not *by*. Things can be replaced by something, but the sentence set up is then "X was replaced by Y", not "replaced X by Y".
...
We replaced ``chunks'' with ``parts'' % on p.@, line@. NC: No need for this bit where it's minor comments and the reviewer has already highlighted the position.
% NC: Sections may also work instead of parts?
...
% NC: Can't believe I missed some of these typos!
...
We modified the sentence to: ``[we] fixed the incorrectly-formatted binomial names (e.g. \textit{H. sapiens} became \textit{Homo sapiens})'' % NC: Note changes I have made to this need to go into ESM.
% NC: Did you also remove the repetition in the next paragraph?
...
\section{Reviewer 2 (Graham Slater):} % NC: I feel like some of the comments are missing here?
\begin{enumerate}
...
This is a great suggestion so we have added this analysis to the ESM @@@% get number.
% We show that mammalian morphological phylogeneticists have focused their energies on resolving the higher level relationships among mammalian lineages.
...
This paper formed part of my PhD thesis that I submitted in September which partially explains the swift submission! % NC: Worth making a tiny joke but no need to show off :P
...
We have clarified this by changing ``observed mean distance between each of $n$ taxa'' to ``observed mean sum of the branch lengths between each of $n$ taxa''. % NC: Fix in document
...
% NC: Let me know if you need help with this.


Thomas Guillerme (guillert@tcd.ie)\\ % Changing email + adding imperial address? % NC: Just change email. For the paper, you can change your corresponding address but I wouldn't add imperial as another proper address as you did most of the work at trinity.
Natalie Cooper
