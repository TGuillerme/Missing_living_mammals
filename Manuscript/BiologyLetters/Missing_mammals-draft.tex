...
% NC: I think anatomical here confuses things as we mean coded anatomical characters. I've tried to clarify.  Also needed to clarify what you mean by "available". Also had to then cut some words to get it back under 200.
...
22 of 28 mammalian orders have \textless 25\% species with available characters; this has implications for the accurate placement of fossils, although the issue is less pronounced at higher taxonomic levels. 
...
% NC: I removed the tip dating bit here, I think yes peter is right in that we are in a special issue on that, BUT it is important for all TEM analyses so adding the tip dating bit here is daft.
\end{abstract}
...
A downside of this method is that it requires molecular data for living taxa and morphological/anatomical data for both living and fossil taxa. %TG: same comment as above. I agree but have added anatomical to make it clear here what you mean when suddenly we switch to using anatomical.
...
However, it is unclear how many coded anatomical characters are actually available for living taxa, i.e. already coded from museum specimens and deposited in phylogenetic matrices accessible online, and how this data is distributed across clades. % TG: emphasize the "availibility" definition her?
...
%
%---------------------------------------------
% NC: I've done a bit of word cutting here as I think we had wordcount issues?
\section{Materials and Methods}
\subsection{Data collection and standardisation}
We downloaded all cladistic matrices %TG: I left "cladistic" here 'cause that's what the matrix is (even following Peter's definition)! % NC: Yep
...
To assess the availability of coded anatomical characters for each mammalian order and across mammals, we calculated the percentage of OTUs with cladistic data at three different taxonomic levels: family, genus and species. %TG: changed cladistic datat to anatomical characters
...
Despite the absence of good morphological/anatomical data coverage for living mammals, %TG: I think leaving cladistic here is OK no? % NC: Better to be consistent with rest of manuscript
...
%It is also encouraging to note that sampling effort is consistent with diversity at each taxonomic level and there is therefore no direct bias in sampling at any taxonomic level (ESM 2, Figure 1).
% NC: Needs more explanation, why is this encouraging??? Maybe leave this as the paper is already over long.
...
%TG: That's two times Graeme though...I've shortened it due to wordcount issues
