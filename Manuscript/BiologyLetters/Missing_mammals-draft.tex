...
\end{center}
\medskip
\noindent{*\bf Corresponding author.} \textit{t.guillerme@imperial.ac.uk}\\  % NC: You need to add your current address to the corresponding author bit I think. TG: i.e. Silwood? That'll eat a shit load of words for not much no? % NC: OK, leave it for now, but you are supposed to use a current address for corresponding author if you have moved. It's leftover from the days people would write you a letter to ask about your paper! Hopefully Biology Letters won't notice :)
\vspace{1in}
...
In most orders, species with available characters are randomly distributed across the phylogeny, which may reduce the impact of the problem. % NC: Sorry can't decide if it is a that or a which :)
...
A downside of this method is that it requires molecular data for living taxa and discrete morphological/anatomical data shared among both living and fossil taxa (i.e. hard tissue characters such as skeletal features). % NC: across or among is correct. Between is only for two things being compared.
...
% NC: Ref 5 - J Evol Biol
% NC: Ref 8. Still that weird "Available from" bit in the ref to our paper. This is not needed and wasting words. Plus looks odd.
% NC: Ref 12 has p. 
% NC: Ref 16 - Now this says KP-g which is wrong! K-Pg or KPg (not sure what it says in the paper title)
