...
\end{center}
\medskip
\noindent{*\bf Corresponding author. Current address: } \textit{t.guillerme@imperial.ac.uk}\\  % NC: You need to add your current address to the corresponding author bit I think. TG: i.e. Silwood? That'll eat a shit load of words for not much no?
\vspace{1in}
...
We do not distinguish between soft and hard characters, but the majority of matrices contain at least some hard tissue characters. % NC: True? This is to deal with the skeletal bit. Put into response letter. TG: probably true. Didn't check all matrices but because of their data's nature (i.e. collected by anatomists back in the days and paleontologists nowadays) I'll bet you there's "at least some".
...
This is analogous to the problem of long-branch attraction/short branch repulsion, as one can think of Herpestidae as having zero-length branches for anatomical characters, and Canidae and Ursidae having long branches and thus ``attracting'' fossil placements. %TG: changed family names here to be consistent (going with always latin).
