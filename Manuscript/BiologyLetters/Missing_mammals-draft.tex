...
\end{center}
\medskip
\noindent{*\bf Corresponding author. Current address: } \textit{t.guillerme@imperial.ac.uk}\\  % NC: You need to add your current address to the corresponding author bit I think.
\vspace{1in}
...
Therefore, there are fewer coded anatomical characters in living taxa, even in well-studied groups like mammals.
...
In most orders, species with available characters are randomly distributed across the phylogeny, that may reduce the impact of the problem.
...
\noindent Key words: Total Evidence method, phylogenetic clustering, cladistic matrix%NC: change cladistic
, extinct, topology\\
...
For example, fossils rarely have molecular data and incomplete fossil preservation may reduce the number of anatomical characters available. %TG: should we add another "(i.e. skeletal characters)" here? % NC: No I think we are making a slightly different point.
...
% NC: OK but need to be careful about just randomly citing ourselves when we know that's not what the paper says.
...
% NC: If someone makes a point about it in a review it means it isn't explained clearly enough. I think adding a note to the methods and/or discussion is needed.
%---------------------------------------------
%
%       METHODS
%
%---------------------------------------------
% NC: From here on in we need to replace the word cladistic. I don't like the "anatomical characters" as it's a bit long. How about character matrices? You decide.
...
We do not distinguish between soft and hard characters, but the majority of matrices contain at least some hard tissue characters. % NC: True? This is to deal with the skeletal bit. Put into response letter.
...
% NC: Can you check the caption we used doesn't have cladistic in it? I have a feeling it does...
...
%
% NC: Yeah I think this is the correct thing to do here.
...
Then we expect two major biases: first, fossils will not be placed within a clade containing no hard tissue data, and second, fossils will have higher probability of being placed within the most sampled clade by chance. % NC: I added hard tissue here
...
% NC: I editted this as the reviewer is concerned that our analyses might be biased, not total evidence in general
...
% NC: The ref for our paper looks weird - it has a url, probably fix that and check the reference is correct so we get the citation.
% Ref 10 - there's an 03 after the date. Not sure what that refers to.
% NC: Ref 16 - is KPg mising a - ?
% NC: Ref 17 has a missing space
