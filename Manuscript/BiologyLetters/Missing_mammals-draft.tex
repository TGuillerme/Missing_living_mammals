...
We downloaded all cladistic matrices %TG: I left "cladistic" here 'cause that's what the matrix is (even folloowing Peter's definition)!
...
NTI \cite{webb2002phylogenies} is based on mean nearest neighbour distance ($MNND$) and is calculated as follows:
  ...
  \end{equation}
where $\overline{MNND}_{obs}$ is the observed mean branch length between each of $n$ taxa with available anatomical characters and its nearest neighbour with available anatomical characters in the phylogeny, 
...
\section{Results}
22 of 28 orders % Change that!
 have low coverage (\textless 25\% species with available anatomical characters) and six have high coverage %Change that!
  (\textgreater 75\% species with available anatomical characters) at the species-level.
...
The number of living mammalian OTUs with no available anatomical characters was surprisingly high at the species-level: only six out of 28 orders have a high coverage of taxa with available anatomical characters.
