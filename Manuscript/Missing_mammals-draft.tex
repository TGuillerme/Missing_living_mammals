...
% because some trees have species, some have subspecies etc.
...
% NC: Explain the genera vs species - ie how you might have more at one level than expected.
...
We designated as ``living'' all the OTUs that where either present in the phylogeny of Fritz \textit{et al}. \citep{FritzTree} or the taxonomy of Wilson \& Reeder \citep{wilson2005mammal}, and designated as ``fossil'' all the OTUs that were present in the Paleobiology database (\texttt{https://paleodb.org} %NC: Check website or reference
...
% NC: START with - small matrices are shit for these reasons.
...
% NC: We also did it with all data.
...
We consider Orders with \textless 25\% of living taxa with cladistic data as having poor data coverage, and Orders with up \textgreater 75\% of living taxa with cladistic data as having good data coverage. 
...
% NC: Put NTI and PD up here too then just describe NRI.
...
At the Genus level, only three orders have \textless 25\% of taxa with cladistic data and 16 orders have less than 75\%.
...
% NC: You don't generally refer to results tables etc in the discussion
...
Therefore, at the species-level within most orders of mammals, we expect to see topological artefacts for the placement of fossil taxa. % NC: Rephrase
