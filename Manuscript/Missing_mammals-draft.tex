...
We also performed a systematic Google Scholar search for matrices that might not have been uploaded on these databases (see see Supplementary Materials for a detailed description of the search procedure).
...
We assigned each species as being either ``living'' or ``fossil'' as follows: (1) we designated as ``living'' all the OTUs that where either present in \citep{FritzTree} or \citep{wilson2005mammal}; (2) we designated as ``fossil'' all the OTUs that were present in the Paleobiology database (\texttt{https://paleodb.org} %NC: Check website
); and (3) for the OTUs neither labeled as living or fossil we tried to decompose the OTUs name (i.e. \textit{Homo$\_$sapiens} became \textit{Homo} and \textit{sapiens}) and tried to match the first element with any taxonomic level (Family, Genus, Species, etc.) from \citep{wilson2005mammal}. % link
...
% NC: Wonder if more can go in supp mat
