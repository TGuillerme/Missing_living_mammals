...
For example, fossils very rarely have molecular data and incomplete fossil preservation (e.g. soft \textit{vs.} hard tissues) may restrict the amount of morphological data available \citep{sansomfossilization2013}. % NC: Worth metioning fossil preservation here briefly as it is also an 
...
We collected cladistic data (i.e. % NC: define cladistic data here
...
matrices containing @9411 unique % Number of unique OTUs
operational taxonomic units (OTUs).
In this study, we refer to OTUs rather than species since the entries in the downloaded matrices were not standardized and ranged from specific specimen names %NC: Do you mean names of individual specimens or subspecies?
to the family-level.
...
Matrices with few characters % NC: matrices can be small in two ways - with few characters and with few species. Here you just mention few characters so I've made it more specific.
...
% NC: Is this true really? Does it always follow that if you've very few characters you also have very specific characters? Maybe so...
...
For each analysis our focal taxa were those with available cladistic data at that taxonomic level and the phylogeny was the phylogeny of the order pruned from \citep{}. % cite  Bininda Emonds?
...
The number of living mammalian taxa with no available cladistic data was surprisingly high at the species-level: only @24 out of @28 orders have a high coverage of taxa with available cladistic data (and two of the 28 orders are monospecific!). % NC: Bit odd to say it's suprisingly high then say there are lots of orders with "high" coverage. I've added an "only" but 24 of 28 sounds quite good to me...
