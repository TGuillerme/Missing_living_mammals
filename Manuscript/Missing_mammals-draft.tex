...
%*** see end of methods for explanation. TG: I've but it below, I think it reads better after explaining the metrics.
Both metrics 
...
% NC: Sorry this still isn't quite right. Check my paper for a nicer way of writing it. - TG: I've done some fixes on the sentence above. I didn't want to copy/paste what's in your paper so I tried to explain it in my own words. Maybe it'll need translation from Thomas-english and will end up sounding similar to what you wrote in your paper. Which is frankly not a big deal since it is the exact same thing we're describing here.
...
% TG: In the end, I don't know if that's useful. For the 10 significant results, only three of them are not significant for both NRI and NTI... Maybe we can decide if we keep it or not after the word count.
...
%We also report the results using another metric, Phylogenetic Distance \citep[PD;][]{Faith19921}, in the Supplementary Information. % TG: In the end is it really important to report PD? There are few problems that I don't understand in the function ses.pd (because I'm a bit lazy and trying to finish other things) such as the fact that the tree must be rooted for some data... Basically it worked well using one of the Fritz tree (which is bad, I know) but when using the Bininda-Emonds tree, I think the fact that it isn't resolved messes up the results and gives most taxa highly significant (clustered or dispersed).
...
\section{Results}
% TG: updated results. Slight changes in which group is significant or not. Mainly due to using Bininda-Emonds tree I'll say. But the main message is still the same. It's badly covered but at least is not that clustered. I just changed the example from Carnivores vs. cetartio to carnivores vs. primates (cetartio are now clustered).
...
% NC: Add a sentence saying this means things might not be quite so bad? Need to justify the "However". however what? TG: I think the "however" fits better below. Here it's §1: "that's what we found"; §2 (first part): "That's baaaad", §2 (second part) "However, it's all right at genus level"
...
Thus, in Total Evidence trees, placements of Carnivoran fossils should considered with caution. %viewed with suspicion. % NC: OK gotta be a better way to say that. TG: is that a bit milder?
...
%Because the data for living mammals is usually readily available in natural history collections, we propose that an increased effort be put into coding morphological characters from living species, possibly by engaging in collaborative data collection projects through web portals such as \textit{Morphobank} \citep{morphobank}. % NC: OK needs a summing up sentence!!! TG: I combined this paragraph with the postive ending point. That might sum up the thing.
Despite the absence of good cladistic data coverage for living mammals, the Total Evidence methods seems to be still the most promising method for combining both living and fossil data and the best way to assess more accurately macroevolutionary processes. % TG: or is that a bit bold?
It is also positive to consider that the problems underlined in this study can be easily solved. %TG: not sure about "easily", the idea is that the solution is not to break the speed of light or something like that, just to continue what people have stopped doing when DNA became trending.
