...
order.structure<-function(order, taxa, tree, reference, metric=c("PD", "NRI", "NTI"), runs=1000, null="taxa.labels", verbose=FALSE, ...) {
   ...
   if(any(metric == "NTI")) {
    ...
    #Looping through the list to select the monospecific taxa
    ...
    #Check if the Order is not monogenerique
    ...
    if(length(mono_test) < 2) {
        ...
        results[2,-c(1,2)]<-rep(NA,ncol(results))
    } else {
        #Tree
    ...
    if(length(mono_test) < 2) {
        ...
        results[1,-c(1,2)]<-rep(NA,ncol(results))
    } else {    
